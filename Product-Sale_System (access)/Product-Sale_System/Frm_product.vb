...
Public Class Frm_product
    ...
    Sub showdata()
        ...
        If cbo_findtype.SelectedValue.ToString = "all" Then
            If txt_findname.Text = "" Then
                strSQl = "SELECT T_products.*,T_type.type_name,T_unit.unit_name FROM (T_products LEFT JOIN T_type ON T_products.type_id=T_type.type_id) LEFT JOIN T_unit ON T_products.unit_id=T_unit.unit_id WHERE T_products.DEL_FLAG = 'N'  ORDER BY pd_id ASC"
            Else
                strSQl = "SELECT T_products.*,T_type.type_name,T_unit.unit_name FROM (T_products LEFT JOIN T_type ON T_products.type_id=T_type.type_id) LEFT JOIN T_unit ON T_products.unit_id=T_unit.unit_id WHERE T_products.DEL_FLAG = 'N' and pd_id LIKE '%" & txt_findname.Text & "%' OR pd_name LIKE '%" & txt_findname.Text & "%' ORDER BY pd_id ASC"
            End If
        Else
            If txt_findname.Text = "" Then
                strSQl = "SELECT T_products.*,T_type.type_name,T_unit.unit_name FROM (T_products INNER JOIN T_type ON T_products.type_id=T_type.type_id) LEFT JOIN T_unit ON T_products.unit_id=T_unit.unit_id WHERE T_products.DEL_FLAG = 'N' and T_products.type_id = '" & cbo_findtype.SelectedValue.ToString & "'  ORDER BY pd_id ASC"
            Else
                strSQl = "SELECT T_products.*,T_type.type_name,T_unit.unit_name FROM (T_products INNER JOIN T_type ON T_products.type_id=T_type.type_id) LEFT JOIN T_unit ON T_products.unit_id=T_unit.unit_id WHERE T_products.DEL_FLAG = 'N' and (T_products.type_id = '" & cbo_findtype.SelectedValue.ToString & "') AND pd_id LIKE '%" & txt_findname.Text & "%' OR pd_name LIKE '%" & txt_findname.Text & "%' ORDER BY pd_id ASC"
            End If
        End If
    ...
    Private Sub btn_save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_save.Click
        ...
        If del = MsgBoxResult.Yes Then
            ...
            conn.Open()
            strSQl = "INSERT INTO T_products(pd_id,pd_name,pd_brand,type_id,pd_cost,pd_price,pd_stock,unit_id,pd_discustomer,pd_dismember,pd_disbirday,pd_picture,DATE_UPDATE,DATE_CREATE,DEL_FLAG) VALUES('" + Trim(txt_id.Text) + "','" + txt_name.Text + "','" + txt_brand.Text + "','" + cbo_typeid.SelectedValue.ToString + "','" + txt_cost.Text + "','" + txt_price.Text + "','" + NUD_stock.Value.ToString + "','" + cbo_unit.SelectedValue.ToString + "','" + txt_discus.Text + "','" + txt_dismem.Text + "','" + txt_disbrith.Text + "','" + namepic + "','" + dtn_EN + "','" + dtn_EN + "','N')"
            mycmd = New OleDbCommand(strSQl, conn)
            Try
